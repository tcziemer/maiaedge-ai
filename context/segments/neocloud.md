...
- `Large Scale GPU - Neocloud`: Lambda Labs, Lightning AI (Voltage Park merger COMPLETED Jan 21, 2026 - operates under the Lightning AI name; reclassify the Voltage Park record), CoreWeave, Nebius, Fluidstack (re-rated 2026: $50B Anthropic capacity program, ~$18B valuation talks - moved up from AI-Infrastructure bucket)
- `Tier 1 Inference - Neocloud`: Together AI, RunPod (boundary), Groq, Cirrascale, DeepInfra, Fireworks AI (~$15B valuation talks), Mistral La Plateforme (boundary), SambaNova, Baseten (~$11B valuation)
...
**Value Prop 5 (multi-cloud interconnect, the front-door hook):**
Your customers move and manage data across AWS, Azure, and GCP from one place, on paths you control, and migrate part of a workload off a hyperscaler without giving up the hyperscaler relationship. This is the value prop that most reliably pulls a demo request from builder and reseller neoclouds. Lead with it when the prospect resells connectivity or cloud access to its own downstream customers (the AI-infrastructure-provider and build-operate models), and pair it with the egress advantage in Value Prop 4.

> **Front-door note (builder / reseller neoclouds).** Observability ("see why you're slow") is the entry for mid-growth inference neoclouds in latency pain. For builder and reseller neoclouds (companies that resell connectivity or cloud access to their own end customers), the entry is cloud interconnect and owning the WAN, not latency debugging. Two patterns recur: (1) cross-cloud data movement and partial workload migration that keeps the hyperscaler relationship intact, and (2) consolidating per-customer networking into one operator-owned layer so the end customer never has to think about it. Both surface as the strongest demo magnets in builder/reseller conversations.
...
| **Infrastructure Lead (all-owner)** | Infrastructure Lead, Head of Infrastructure, Director of Infrastructure (often ex-Arista / Dell / Cisco) | The single technical person who owns everything hard at a builder neocloud: cluster architecture, network design, customer access, professional and managed services. Provisions nodes and designs the fabric himself, and reasons in BGP, ASN, HA, and single-point-of-failure terms unprompted. Self-discovers product value rather than needing to be sold (one buyer reframed the product into "can I build my own private net" mid-call). Primary technical champion at builder / integrator neoclouds. Cannot sign alone (takes it back to the team), so pair with an economic buyer. |
| **Commercial / BD Champion** | VP Sales, VP BD, VP Partnerships, GM, President | Partner-relationship and economics-minded exec, not a network buyer. Drives the evaluation around co-sell and cloud connectivity, and frames every value prop as "can I sell this to my customer." The channel gatekeeper: wants MaiaEdge as the enablement layer behind their offer, not a vendor going direct to their customers. Lead with the co-sell / customer-readiness angle and the egress economics they can resell, not fabric specs. |
...
**Provider-survival / diligence hooks** (any maturity selling INTO enterprise; the buyer's bubble anxiety is the opening):
The macro shift: enterprise buyers of neoclouds now run provider-durability diligence  -  "will this provider still be here, and can it prove its infrastructure holds." The network is the part of that answer most neoclouds cannot currently show. Owning the path and proving it hop by hop turns a macro anxiety into the neocloud's own differentiation  -  it is how the neocloud ANSWERS the diligence question rather than hoping it isn't asked.
- "Your enterprise buyers are running provider-durability checks now, and the network is the part of the answer you cannot currently show them. Owning the path, and proving it hop by hop, is how that diligence question closes in your favor."
- "When a buyer asks how you stay up and stay accountable for the network under your service, hop-by-hop proof on paths you control is the difference between a yes and a longer evaluation."
- "The bubble question is being asked in your deal cycles now. The neoclouds that win answer it with infrastructure they own and can prove, not assurances."
...
|-------------|----------|------------|-------------------|-------------------------------|
| **Large Scale GPU - Neocloud** (19 records) | Nebius (Q1 2026 rev +684%; Meta $27B/5yr), Lambda Labs (IPO-speculated, no firm date; multibillion Microsoft deal), Fluidstack (IPO ~2027; $50B Anthropic capacity program), Lightning AI (Voltage Park merger completed Jan 21, 2026 - reclassify record), CoreWeave (ClusterMAX Platinum; 49 DCs, >1GW active, $99.4B backlog, building its own encrypted fiber backbone - the network-becomes-product proof) | Multi-facility GPU-as-a-service, 20+ facilities, >100MW disclosed GPU capacity, bare-metal GPU clusters | Observability across distributed training clusters, deterministic inter-facility paths | `Large Scale GPU - Neocloud` |
| **Tier 1 Inference - Neocloud** (23 records, default T2) | Together.ai (25+ cities, 200MW; $305M round), Groq (35 Equinix POPs; NVIDIA acqui-hire Dec 2025 $20B), Cirrascale, DeepInfra ($107M Series B May 2026), Fireworks AI (~$15B valuation, in talks), Mistral La Plateforme (boundary - foundation-model lab with own inference API), SambaNova (SN50 Feb 2026, 405B record), Baseten (~$11B valuation) | Distributed inference endpoints at 20-50+ edge locations, sub-100ms token latency SLAs | Real-time telemetry for latency diagnosis, multi-carrier orchestration between edge POPs | `Tier 1 Inference - Neocloud` |
...
- EU AI Act: GPAI and transparency obligations apply August 2026, but the Digital Omnibus (provisional agreement May 2026) deferred the high-risk (Annex III) obligations to December 2027. Lead sovereign urgency on data-residency demand and sovereign-RFP premiums, not on a contested fine deadline (fines still reach 7% global revenue once the high-risk rules bite)
...
| **"Our enterprise buyers are worried about whether neoclouds will even survive"** | That diligence question is the opening, not the obstacle. Provider-durability checks are now standard in enterprise neocloud deals, and the network is the part of the answer most providers can't show. Owning the path and proving it hop by hop is how you answer "can you stay up and stay accountable" with infrastructure instead of assurances  -  it turns the bubble anxiety into your differentiation. |
| **"Who are you?"** | Same team that built Acme Packet ($2.1B to Oracle) and 128 Technology ($450M to Juniper). Two exits, $2.5B+ combined. The product is carrier infrastructure that operators and AI clouds deploy on their own networks. |
...
H100 pricing fell sharply from late 2024 through mid-2025 (cloud rental $8-10/hr Q4 2024 to ~$2.99/hr Q1 2026 spot, AWS H100 spot down up to 88%), but the trend has REVERSED. H100 1-year contract pricing climbed +40% from October 2025 to March 2026, from $1.70/hr to $2.35/hr. The prior assumption that GPU prices were collapsing in a straight line has been invalidated. NVIDIA Blackwell sold out through mid-2026 and B200/GB200 allocation pressure is pulling H100 reserved capacity tighter. TSMC expanding CoWoS capacity to 120-130K wafers/month by late 2026 (up from 75K) may ease supply H2 2026, but contract pricing has already turned. Midjourney moved from NVIDIA to Google TPU v6e, cutting inference spend from ~$2.1M to under $700K/month  -  still a cautionary tale for GPU-only neoclouds, but the underlying economics have shifted from margin compression to margin recovery for operators with locked-in capacity. March-June 2026 update: the reversal PLATEAUED at recovered levels  -  hyperscaler H100 on-demand pricing was flat Mar 1-Apr 20 (0.47% coefficient of variation), on-demand averages ~$3.64/hr (+4% YoY), the market clusters at $2-4/hr, NVIDIA raised H100 rental pricing ~20% in 2026, and sub-scale operators who can't break even at ~$1.65/hr started exiting (the "Great Neocloud Consolidation": Voltage Park into Lightning AI, Northern Data into Rumble, smaller exits through Q1-Q2). Margin-healthy survivors are expansion-minded again  -  which raises the cost of a colo or carrier losing one.

### Inference Overtaking Training (the flip happened)
Inference now represents ~60-70% of AI compute (mid-2026), up from ~33% in 2023, with 70-80% projected by END of 2026 - not 2030. Inference infrastructure spend passed training spend for the first time. The serving layer is where capital migrated: serverless-inference platforms re-rated hard in H1 2026 (Baseten $200M→$600M ARR in one quarter; Fireworks ~$800M annualized). Training tolerates retries. Inference doesn't  -  network quality directly impacts customer experience, and agentic workloads multiply the calls.

### The GPU Debt Wall (sized, rated, and CFO-literate)
GPU-backed debt industrialized in 2025-26: Moody's flags a ~$662B AI data-center funding wall; JPMorgan projects $30-40B/yr of data-center securitization through 2027; CoreWeave runs an $8.5B GPU-backed facility rated A3; sub-6% rates exist for hyperscaler-anchored builds. Useful-life assumptions are a public battleground (4/5/6-year lives defended on earnings calls). Must keep GPUs utilized to service debt  -  network downtime = checkpoints = missed debt service. "Uptime as collateral protection" is now a CFO-literate frame, not a metaphor.

### The Networking Audit Went Public (April 2026 - the year's most weaponizable artifact)
Omdia, "Neoclouds Not Ready for AI Networking" (April 15, 2026): an audit of 50 neoclouds found 43% urgently hiring network engineers, more than half not peering at internet exchanges, 46% controlling only small IPv4 blocks, 1 in 5 single-homed on a single transit provider, and over a third minimizing contractual liability for network SLAs - with enterprises explicitly advised to scrutinize suppliers' network posture beyond raw compute. Third-party proof of every discovery question we ask: transit diversity, peering, accountability, the last hop. Use it as the catalyst for technical seats the way the fabric-compute pivot serves commercial seats (seat calibration per the Tier 1 Inference correction). Companion datapoints: GTC 2026 reframed the business as token economics ("Revenue = tokens-per-watt x available gigawatts" - the network is the term most teams can't tune), and the flagship third-party fabric raised ~US$594M (A$827M, June 3, 2026) to build a distributed inference cloud because "serving rewards proximity and distribution" - their words, validating the path-to-the-user thesis. The raise cuts both ways: the same capital that proves the thesis also turns that on-ramp fabric provider into a direct competitor for this buyer, building the distributed inference layer the neocloud is trying to own. It is both validation and a competitive flag (the on-ramp incumbent moving into compute) - see `context/core/competitive-positioning.md` "The On-Ramp Incumbents Became Compute Competitors."
...
### Time to Power (the constraint behind the constraint)
Before allocation, before GPUs, the gating scarcity is power. Energizing a new site is a multi-year grid-interconnect-queue fight, and the operators who win that fight have already paid the hardest cost in the buildout. That changes the economics of network-driven utilization loss. Best-effort connectivity between a hard-won energized site and the rest of the fleet quietly hands back the capacity they spent years to secure  -  the same waste, but now measured against power that was scarce, queued, and expensive to land. Power scarcity RAISES the cost of every utilization point the network leaves on the floor. This is an internal angle-selector for the CEO/Founder seat (they lived the grid queue); do not cite specific MW figures or grid-queue durations in cold copy  -  reference the years-of-waiting experience, then pair it with paths their team controls and provisions in minutes.
...
- GPU utilization as THE metric that determines profitability (tokens-per-watt is the 2026 board phrasing)
- Inference revenue mix and pricing strategy now that contract rates recovered and plateaued high ($2-4/hr market band)
- Debt servicing  -  keeping clusters running against a ~$662B industry funding wall and GPU-backed facilities
...
| Inference overtaking training (~60-70% of compute, 70-80% by end-2026) | Inference is latency-sensitive and distributed  -  network quality directly impacts customer experience | "Training tolerates retries. Inference doesn't. Your customers feel every jitter spike." |
| GPU Debt Wall (~$662B funding wall; GPU-backed facilities; rated paper) | Must keep GPUs utilized to service debt -- network downtime = checkpoints = missed debt service | "Every network interruption forces a checkpoint rollback. Downtime isn't just costly when debt is mounting -- it's existential." |
| The Omdia networking audit (50 neoclouds, April 2026) | Single-homed transit, no IXP presence, thin IP holdings, nobody contractually answering for the network -- now board-visible findings enterprises are told to check | "The audit your buyers read grades the network now. Owning the path, and proving it hop by hop, is how the finding closes." |
...
| Power is the gating scarcity (multi-year grid-interconnect queues to energize a site) | The hardest-won asset in the buildout is the energized site; best-effort paths between it and the fleet hand that capacity back, and power scarcity raises the cost of every utilization point lost | "You waited out a multi-year grid queue to energize that site. Best-effort paths between it and the rest of your fleet quietly hand some of that capacity back, and your team provisions deterministic paths in minutes instead of waiting on a carrier to fix it." |
...
- "Inference is two-thirds of our compute now. Our back-end fabric was built for training."
- "Tokens-per-watt is the number the board asks for. The path to the user is the term we can't tune."
- "We're one transit provider deep in two regions. If the audit question comes, we don't have an answer."
