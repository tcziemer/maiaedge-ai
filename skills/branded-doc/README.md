...
The canonical docs are generated by `build.py` from their markdown sources in `context/partner-assets/` (`maiaedge-101.md`, `cheatsheet-<segment>.md` for the 6 segments, `product-quick-reference.md`) - read those sources for structure. For a visual match, open whatever approved PDFs are currently in `partner docs/final/`; if the generated set (`MaiaEdge-101.pdf`, the cheat sheets, `Product-Quick-Reference.pdf`) is not present, regenerate it with `python3 build.py`.
