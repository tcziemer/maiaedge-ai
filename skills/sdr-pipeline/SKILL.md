...
The single biggest failure mode in this skill is writing email bodies before doing the research, then back-filling weak "Public Signal Cited: NONE — inferred angle" lines to look compliant. That is research-skipping in disguise. At batch scale, even one round of skipped research means dozens of generic emails. Our entire strategy hinges on relevance, and relevance is impossible without paired company + contact research.

**Hard gate (Step 7d below):** Before writing any email body for a contact, you must emit a complete **Research Receipt** for that contact. The Receipt requires literal search query strings that you actually ran. "NONE" alone is not enough — NONE must be paired with the literal queries that were tried and returned no Tier A or Tier B hits, with a minimum of 5 queries.

**Self-check before writing every email:** Look at your output above the email body for THIS contact. Do you see a Research Receipt with at least 3 literal queries paired with results, plus a contact-level finding? If no, STOP. Run the searches now. Then come back and write.

**At batch scale, the audit lives in Sheet 3 of the review XLSX.** Sheet 3 has dedicated columns for the Receipt fields. Cooper scans Sheet 3 to spot research-skipping — a high NONE rate paired with thin contact-level findings is the pattern that flags a batch as research-skipped. If you cannot fill the Sheet 3 Receipt columns truthfully, do not include the contact in the Smartlead output.
...
3. **Read messaging references.** Before writing any emails or LinkedIn messages, read these reference files:
   ...
   - `references/email-writing-rules.md`  -  Email structure, CTAs, quality checklist, banned phrases, segment lock rule
   ...
   - Segment cheatsheets for the relevant segment (colocation.md, fiber-operator.md, neocloud.md, network-operator.md, msp-aggregator.md)
...
The OLD generic searches (`[Company] recent news 2025 2026`, `[Company] funding round`, etc.) are deprecated. They produced unstructured findings that the writer then had to reverse-engineer into a signal. The catalog-pattern searches produce structured findings that are signals by construction.
...
Before any email body is written for this contact, emit a complete **Research Receipt** block. This is a hard gate. No Receipt = do not write the email. The Receipt is what proves research was actually run and not back-filled after the fact.
...
**Why this format enforces what it does:**
- **Literal queries** make faking research more expensive than running it. Writing 3-5 specific query strings is roughly the same effort as running them — but only one produces real findings.
...
- **Contact-level finding is its own required line.** The old "Public Signal Cited" block let writers fold contact research into the company line or skip it entirely. A separate line forces Step 7b's contact-level tailoring to actually happen, not just be claimed.
- **Each query gets its own result line.** Listing queries without per-query results fails the format.
...
- Short, direct sentences. Fragments are fine. One thought per sentence.
- Confident but not aggressive, direct but not polished. Speak at eye level.
- When uncertain, say so honestly. "I'd guess" and "I'd imagine" used genuinely.
- One core idea per message. Commit fully, no hedging or stacking backup points.
- Research fades into voice. Reader should feel you understand their world, not that you processed them through a system.
- Some sentences should just sound like someone talking. Not every sentence does "work."
...
- **Value bridge (AT MOST 1 sentence, embed when possible):** Two valid placements:
  ...
  BANNED: multi-sentence value bridge paragraphs. BANNED openers: "MaiaEdge is..." / "We help operators…" / "We built infrastructure that…" / "We work with…" Use "I" voice, not "we" voice. Max 1 product-specific term per email.
...
- NO brand-voice constructions ("We help operators…" — banned in LinkedIn DMs too).
...
This is not optional. The ITW batch showed measurable quality degradation after batch 5.
...
- **Colo Standard:** "Build your own fabric. Automated virtual cross-connects, a services layer you can productize, and cloud on-ramp under your brand  -  without a multi-year development project or a hyperscale facility build."
- **Colo AI:** "GPU tenants deploy dense interconnection fast. The connectivity layer either keeps up or it becomes the gap in the facility. Deterministic paths between AI sites, automated cross-connects, cloud on-ramps for GPU workloads."
- **Fiber:** "Monetize the fiber you already have. Stand up an instant private fabric across your network over any transport  -  no routing complexity  -  and start selling services you couldn't before, including cloud on-ramp."
- **Network Op Track A:** "You've automated internally. Extend that reach everywhere else."
...
- **Neocloud (in-pain-now):** "Inference latency varies by facility and your team is guessing whether it's the carrier, the colo, or something in between."
- **Neocloud (scaling-wall, 15+ sites hyperscaler-heavy):** "The first 5 hyperscaler contracts didn't need a network team. The next 40 enterprise customers will."
- **Neocloud (early-growth, crypto-to-AI):** "Bitcoin doesn't care about latency. Enterprise AI tenants do. The connectivity that worked for mining doesn't survive an inference SLA."
