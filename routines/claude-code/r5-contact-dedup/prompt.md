...
Write this structured report to the on-disk run report at `weekly-reports/YYYY-MM-DD/r5-contact-dedup/run-report.md`. It is NOT a DM body (see Delivery).

- **Subject (use as the report's top heading):** `CRM Guardian - Contact Dedup (Weekly) - [YYYY-MM-DD] - [N] flagged, [M] Tier 3 held`
...
Do NOT DM Cooper a per-run debrief. On a clean or partial-but-recoverable run, the full record is: (1) the on-disk run report at `weekly-reports/YYYY-MM-DD/r5-contact-dedup/run-report.md` (the Output structure above is that report, not a DM body; it carries the bulk-delete pointer for review), and (2) the one Run-log row this routine already appends to the working-ledger canvas `F0B0AFSB9LN` (status emoji from the canvas conventions). The CRM Ops Daily Digest (M-F 4:45pm CT) surfaces this run's work from HubSpot + the ledger, so a self-DM is redundant.

Send a Slack DM to Cooper (`U0A24D9RJLS`, self-DM, workspace `maia-edge.slack.com`) ONLY on a hard failure - HubSpot or Slack MCP unreachable, an abort, or zero contacts processed against a non-empty duplicate-group queue - as ONE line:
`:red_circle: CRM Guardian - Contact Dedup (Weekly) [FAILED/ABORTED/PAUSED] - [one-clause reason].`
