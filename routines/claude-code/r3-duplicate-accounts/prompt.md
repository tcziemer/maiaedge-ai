...
**Connected tools:** HubSpot MCP, Slack MCP (canvas Run-log append + failure-ping only; no per-run debrief DM - see Delivery). No Apollo.
...
### F. Idempotency
Write `customer_segment = "Flagged for deletion"` only AFTER contacts have been reassociated. In the SAME update, set `flagged_for_deletion_reason` leading with the canonical code `Duplicate (merged)` (see §2.1 of `context/hubspot/property-schema.md` for the 7-code spec), followed by a colon and one sentence citing the primary/keeper record (name + HubSpot ID) — e.g. `Duplicate (merged): contacts reassociated to primary Acme Corp (company 12345); flagged loser.`. No em dashes in the reason string. Idempotent: a second same-day run sees drained, flagged duplicates and returns "All clean."

### G. Write Authorization
`confirmationStatus = "CONFIRMATION_WAIVED_FOR_SESSION"`.

**Pre-authorized writes on duplicate records:** `customer_segment = "Flagged for deletion"` + its mandatory companion `flagged_for_deletion_reason` (set in the same update; code `Duplicate (merged)`) — this is the one exception for this routine, gated by the Mode A workflow completing successfully and Customer Protection not firing.
...
4. After ALL contacts reassociated successfully: set `customer_segment = "Flagged for deletion"` on the duplicate (Tier 1), and in the SAME update set `flagged_for_deletion_reason = "Duplicate (merged): contacts reassociated to primary <primary name> (company <primary ID>); flagged loser."`. The scannable code lives in this field; leave any existing `account_brief` prose intact. No em dashes in the reason.
...
Write this structured report to the on-disk run report at `weekly-reports/YYYY-MM-DD/r3-duplicate-accounts/run-report.md`. It is NOT a DM body (see Delivery).

- **Subject (use as the report's top heading):** `CRM Guardian - Duplicate Accounts - [YYYY-MM-DD] - [N] pairs consolidated, [M] Tier 3 held`
...
Do NOT DM Cooper a per-run debrief. On a clean or partial-but-recoverable run, the full record is: (1) the on-disk run report at `weekly-reports/YYYY-MM-DD/r3-duplicate-accounts/run-report.md` (the Output structure above is that report, not a DM body), and (2) the one Run-log row this routine already appends to the working-ledger canvas `F0B0AFSB9LN` (status emoji from the canvas conventions). The CRM Ops Daily Digest (M-F 4:45pm CT) surfaces this run's work from HubSpot + the ledger, so a self-DM is redundant.

Send a Slack DM to Cooper (`U0A24D9RJLS`, self-DM, workspace `maia-edge.slack.com`) ONLY on a hard failure - HubSpot or Slack MCP unreachable, an abort, or zero pairs processed against a non-empty HIGH-confidence candidate queue - as ONE line:
`:red_circle: CRM Guardian - Duplicate Accounts [FAILED/ABORTED/PAUSED] - [one-clause reason].`
