# CRM Guardian - Routine 2: Stale Account Re-Enrichment (HISTORICAL Claude Code copy)

> **Status: DISABLED / NOT SCHEDULED.** R2 moved to Cowork during the 2026-04-30 platform split because the deep-research workflow depends on `web_fetch` against arbitrary domains, which Claude Code's egress proxy blocks. The live operational version is at [`cowork-scheduled-tasks/r2-stale-reenrichment/prompt.md`](../../../cowork-scheduled-tasks/r2-stale-reenrichment/prompt.md). This Claude Code copy carries the Phase 3 operating principles for reference; if the egress block is lifted and Cooper wants R2 re-enabled on Claude Code, sync this prompt to the Cowork version's content first (the Cowork copy implements Step 0B MISDOMAIN check, account_brief regeneration on every FULL pass, recent_news_or_trigger_event 90-day staleness clearing, and the Greenfield auto-migration F.2 rule that the 2026-05-03 redesign added).
>
> **Phase 3 deltas (synced to Cowork canonical 2026-05-14):**
> - Canonical sub-segment vocabulary: 30 active values in `context/account-tiering/sub-segment-qualification.md`. Includes `Subsea cable operator` (new 30th, 2026-05-14), `Crypto to AI - Neoclouds` (inclusive of operator AND landlord per Cooper 2026-05-14), `Greenfield` (REAL sub-segment, not deprecated). Auto-migrate legacy values on read: `Tier 1 Carrier - Network Op` (replaces "Tier 1 Global Incumbent"), `Managed Network Services - MSP` (replaces `- Network Operator` suffix).
> - D5 v2 protocols inlined at `context/account-tiering/enrichment-protocols.md` §6 (30 protocols), §6a (NC1/NC3/NC2 threshold matrix - REQUIRED for NeoCloud re-classification), §7 (Greenfield 4-tier migration catalog - REQUIRED for R2's F.2 Greenfield auto-migration). Do NOT reference `working/D5-enrichment-protocols-no-silent-failures.md` - it is historical source only.
> - Account tier computation: `context/account-tiering/tier-compute-spec.md` is canonical. Stage 4 read; tier is COMPUTED, not assigned.
...
> - `maiaedge_value_proposition` is OUT OF ENRICHMENT SCOPE per Cooper 2026-05-14 (Operating Principle #6). R2 does NOT write it. The Pre-authorized writes block below has been corrected.
> - 2-4 sentence conciseness cap on narrative enriched fields (`account_brief`, `provisioning_landscape`, `recent_news_or_trigger_event`).
> - `last_enriched_date` policy (CLAUDE.md unified table): RE_ENRICH_FULL + gate pass = YES bump; RE_ENRICH_LIGHT eviction (PARTNER_KEEP / HARD_DELETE / DEAD_DOMAIN) = YES; Tier 3 / inconclusive / partial = NO. Signal Scan Stage 5 partial writes do NOT bump (Signal Scan owns its own freshness via signal fields, not last_enriched_date - this guarantees R2's 120-day rotation still owns the every-account audit).
> - Apollo budget tracker: read `weekly-reports/apollo-budget.json` at run start per `routines/_shared/apollo-weekly-budget-spec.md`. R2 sub-cap = 30/run. Global cap = 850/week.
> - Greenfield auto-migration is OWNED by R2: when a record has `company_sub_segment = "Greenfield"`, R2 checks `recent_news_or_trigger_event` and the broader signal corpus for operational state transitions per `context/account-tiering/enrichment-protocols.md` §7 (4-tier pattern catalog: Tier 1 operational milestone -> 20+ phrases; Tier 2 abandonment -> 13 phrases; Tier 3 construction-progress -> 11 phrases that keep Greenfield; Tier 4 stalled -> D7 fallback).
...
|---|---|---|
| `RE_ENRICH_FULL` | Already classified as ICP (Colocation / Fiber / NeoCloud / Network Operator / MSP-Aggregator / **Enterprise-CustomerSegment**) AT ANY TIER, OR currently `customer_segment IS EMPTY` with a usable domain | Full re-enrichment pipeline (Stages 1-3 + Apollo). Apply Non-ICP Eviction Rule if classification flips to non-ICP. |
| `RE_ENRICH_LIGHT` | Already classified as `Other` or `Unknown`, AND `account_tier` in {TIER_4, TIER_5, UNRANKED} | Eviction-decision path (no Apollo): fetch domain, apply Non-ICP Eviction Rule decision tree. **Most "Other" records will resolve to PARTNER_KEEP (bump date) or HARD_DELETE (flag for eviction).** This is where stale Tier 5 pollution gets cleaned up. **`Enterprise-CustomerSegment` records REMOVED from this bucket 2026-05-11** - Enterprise is now ICP. Pre-promotion Enterprise records at TIER_5 / `low_5069` confidence get a dedicated scale-gate recheck routed to RE_ENRICH_FULL - see cowork prompt's `RE_ENRICH_LIGHT - Enterprise scale-gate recheck` sub-bucket. |
...
These are records currently classified as `Other` or `Unknown` at low tiers (TIER_4 / TIER_5 / UNRANKED). The point of this path is to verify each is STILL a legitimate keeper (Partner Target) or finally flag-for-deletion the ones that aren't. Stop letting Tier 5 sludge sit in the CRM forever. **Note (2026-05-11):** `Enterprise-CustomerSegment` is now ICP, NOT a RE_ENRICH_LIGHT bucket. Enterprise records ALWAYS route to RE_ENRICH_FULL (see Step 0 bucket table above for the dedicated `RE_ENRICH_LIGHT - Enterprise scale-gate recheck` sub-bucket that promotes Enterprise to FULL re-enrichment).
