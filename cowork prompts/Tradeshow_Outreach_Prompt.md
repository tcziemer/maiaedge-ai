...
MaiaEdge is attending [EVENT]. These instructions govern how to process a contact list into send-ready outreach.

The motion: account-level research once per account, per-contact pass to flex the angle to the role. The work is in finding what's specific to each company AND each contact. Tradeshow attendance is the opener anchor; relevance from research is what earns the read.
...
1. **Event name** (e.g., ITW, PTC, Metro Connect, Fiber Connect)
2. **Event dates and venue/city** (used in CTAs and Email 3 timing language)
3. **Subject line** — locked in per run, all lowercase, no variation. Configured in Smartlead, not stored in the output file.
4. **HubSpot event hashtags** for shared-event matching on warm contacts (e.g., #ITW26, #PTC26, #MetroConnect26)
5. **Source file path and sheet name**
6. **Output file name** (default: `[ListName]_Written.xlsx`)
7. **Create HubSpot LinkedIn tasks: yes or no?** — ASK before the run starts. If yes, create a HubSpot task on each contact for the rep to send the LinkedIn message. See "LinkedIn Task Structure" below.
...
| 8 | LinkedIn Message | Target 35-50 words / max 280 chars (under LinkedIn's 300 hard limit). NO sender intro in body. Research Receipt above (see skills/cold-email/SKILL.md and skills/linkedin-outreach/SKILL.md for the four-section format). Written for **every contact** in tradeshow runs. |
...
**Subject line:** configured in Smartlead, not stored.

**Skipped contacts:** include row with First Name through Account populated. Put skip reason in Email 1 (e.g., `SKIPPED: Active deal`). Leave Email 2, Email 3, LinkedIn blank.
...
**Layer in 2-3 web searches:** recent news, expansions, partnerships, hiring, funding, route launches, facility announcements. Validate or update anything in the brief that may have changed. Confirm or override the segment.

**For Active accounts** (Activity Status flagged): pull HubSpot call notes, email threads, meeting notes, deal status, task history. Look for shared event activity using the per-run hashtags. If real history exists, use it — "We've connected with a few folks at [Company]" or "I think our teams were both at [past event] and didn't get a chance to connect." If activity is thin, don't force warmth.
...
4. Write 1 LinkedIn message (always, for tradeshow runs)
...
- **Warm angle authenticity:** HubSpot-backed only
- **Mechanical:** no em dashes, no banned phrases, no competitor/customer names, no credibility anchors, correct sender per Account Owner, correct event/venue
- **Word count:** Email 1 ~70-85 words; Email 2 under 55; Email 3 short
- **First name** before email body
- **LinkedIn:** Target 35-50 words / max 280 chars (under LinkedIn's 300 hard limit), NO sender intro in body, company-specific problem, soft CTA optional, Research Receipt above (see skills/cold-email/SKILL.md and skills/linkedin-outreach/SKILL.md for the four-section format)
...
1. **Event hook (1 sentence):** natural reference. For confirmed attendees: "saw you in the app." For lists: "saw [company] on the exhibitor list" or similar. For ACTIVE contacts with HubSpot event history: "I think our teams were both at [past event] and didn't get a chance to connect." The event hook IS a public-signal observation — cite it in the Company-level finding line of the Research Receipt.
2. **Problem/angle pivot (1-2 sentences):** the company-specific angle from research. Recipient should think "this person understands what we're dealing with" without seeing a single fact about their company stated back at them.
3. **Value bridge (1 sentence MAX, embed-by-contrast preferred):** how MaiaEdge relates. Segment vocabulary only. Preferably embed as a contrast clause inside the problem paragraph ("the fix is…" / "the version that works is…"). Standalone allowed only as a 1-sentence "I" voice line. NO brand-voice constructions ("We help operators…" / "We built infrastructure that…" — BANNED). Multi-sentence value bridge paragraphs BANNED.
4. **CTA (1 sentence):** event-anchored. Optional when an illumination question carries the close.

**Posture (DIRECT or ASKED):** Match to signal strength. Event opener + cataloged signal → DIRECT. Event opener + inferred angle → ASKED.

**Research Receipt** required above the email body — four sections: Searches Run (≥3 literal queries paired with results, ≥5 if NONE), Company-level finding, Contact-level finding, Posture with reason. Event registration / exhibitor list counts as a public signal — cite it in the Company-level finding line. See `context/outreach/email-writing-rules.md` for the canonical Receipt format.

**CTA:** "Open to setting some time up at the show?"
...
1. **New thought (2-3 sentences):** genuinely different angle on the same underlying problem. Peer observation, market shift, different facet.
2. **CTA (1 sentence):** rotated, opens the door to a delegate.

**CTA:** "Would be great to find time to sit down while we are all at the show, or happy to start with someone on your team. Either way works."

### Email 3 (2-3 sentences max)

The event is around the corner — casual nudge.

1. **Timing nudge (1-2 sentences):** event coming up, casual and human.
2. **Soft close (1 sentence):** door open. ONE CTA. Never conditional close + a second close.

**CTA options (rotate for variance, swap event/venue per run):**
- "If this is worth a conversation, happy to find time at the show."
- "Either way, hope to cross paths at [event]."
- "If the timing's right, let's grab a few minutes in [city]."
- "Happy to pick this up at [event] if it makes sense."

### LinkedIn Message (Every Contact)
...
**NO sender intro in body.** Recipient sees who sent the connection request from LinkedIn's UI. "Tim from MaiaEdge." / "Ken from MaiaEdge." in the message body is BANNED — it's redundant and triggers the sales-pitch reflex.
...
**Soft CTAs:**
- "Worth connecting?"
- "Can we find time to connect at the show?"
- "Worth connecting at the show?"

Event can be the opener ("Saw you're heading to [event]") OR the CTA anchor ("at the show") — not both. The "saw you're heading to [event]" opener is itself a public-signal observation when the contact's LinkedIn or registration is public.

**Open with the recipient's first name** followed by a comma, then directly into the observation/question.

**Avoid:** generic "let's connect at [event]," compressed-email format, flattery, listing MaiaEdge features, more than one MaiaEdge sentence, brand-voice constructions ("We help…" / "Most operators…"), sender intro in body.
...
**No fake familiarity.** Warmth comes from real shared history (HubSpot-verified) only. A direct cold approach beats forced familiarity.
...
2. **Work the warm angle if real.** If genuine interaction history exists, reference it naturally.
3. **Check shared event history** using per-run event hashtags. If found: "I think our teams were both at [event] and didn't get a chance to connect."
4. **Active deal(s) contacts:** review the deal. May SKIP. May need to complement existing sales conversation.
