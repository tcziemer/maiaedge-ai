...
- `Enterprise` (legacy naming — represents MSP / Aggregator)
- `NeoCloud`

All other values (`Dark Fiber - Commercial Enterprise`, `Enterprise-CustomerSegment`, `Partner Target`, `Other`, `Unknown`) are out of ICP for the purposes of this skill and eligible for flagging. `Flagged for deletion` is terminal — the skill never re-enters the pipeline on these unless a human explicitly requests re-evaluation.
...
- Set `customer_segment = "Flagged for deletion"` on a company once all its contacts have been resolved (reassociated, preserved-and-left, or flagged)
...
```
name, domain, website, customer_segment, account_tier, hubspot_owner_id,
...
3. Set the duplicate company `customer_segment = "Flagged for deletion"` (TIER 1 — once contacts are resolved, the duplicate can safely be flagged).
...
2. **Post-loop decision for the company:**
   - If ALL contacts were FLAG (zero preserved): set company `customer_segment = "Flagged for deletion"` (TIER 1). Safe to flag — nothing active on it.
...
| Duplicate | Primary | Contacts Reassociated | Contacts Flagged (dup) | Duplicate Company Flagged |
|-----------|---------|-----------------------|------------------------|---------------------------|

MODE B — STANDALONE FLAGS (applied, Tier 1):
| Company | Owner | Contacts Flagged | Contacts Preserved | Company Flagged? |
|---------|-------|------------------|--------------------|--------------------|
...
- `manage_crm_objects` (updateRequest):
  - Write `customer_segment`, `flagged_for_deletion`, `hubspot_owner_id` values
