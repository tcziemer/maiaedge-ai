# CRM Guardian — Routine 0: Import Validator
...
### F. Idempotency
After processing a record, set `last_enriched_date` ONLY if Routine 0 took a write action (rename or flag). Otherwise leave `last_enriched_date` blank so Routine 1 picks the record up at 6 AM. Idempotency key: a second same-day run sees nothing new in the 24-hour `createdate` window after the first run drains it.

### G. MaiaEdge Gotchas
- `account_tier` inverted (Tier 1 = highest).
- Never confuse legacy MSP/Aggregator companies with the literal English word "Enterprise" — the schema-correct value `Enterprise` is MSP/Aggregator (Routine 0 should not act on this).
...
- **Blockchain / crypto / NFT projects** (ledgerofearth.com — unless explicitly a NeoCloud crypto-pivot per segments/neocloud.md "Crypto-to-AI Pivots")
