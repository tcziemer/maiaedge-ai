...
## Margin-Compression P&L Frame (Standard colo CFO / CRO)

Pairs the existing "just a landlord" positioning with the P&L reality the finance seat actually feels. Power costs keep climbing, and most of that increase gets passed through into the space-and-power line - but only partially, because tenants resist full pass-through and renewals reset on competitive rates. The result is space-and-power margin that thins every year while the power bill grows. Interconnection revenue is the line that escapes this: cross-connects, virtual meet-me rooms, and cloud on-ramps are priced on connectivity value, not indexed to the kilowatt. The operators pulling ahead are leaning on the high-margin interconnection line precisely because it holds its margin while space-and-power does not. The "just a landlord" frame is a P&L frame, not only a strategy frame: a landlord's only line is the one getting squeezed.

**When to use:** CFO / CRO / VP Commercial buyer at a Standard colo; renewal-pricing pressure or rising power costs are publicly discussed; interconnection is a small share of revenue. Internal triggers (do not cite in cold): interconnection attach rate, the 18%+ interconnection-revenue benchmark from `colocation.md` relevance bridges, retail asking-rate trend.

**Cold-safe line (CFO/CRO margin frame):** "Power costs keep climbing and most of that lands in space-and-power margin you cannot fully pass through, so the operators pulling ahead are leaning on interconnection revenue that is not tied to the power bill. Your team can stand up self-service cross-connects and cloud on-ramps under your brand to grow the line that holds its margin."
...
> **⚠️ Internal triggers - do NOT cite in cold.** The vacancy, pricing, and YoY figures below are angle-selection triggers, not customer-facing talking points. Latest read (CBRE H2 2025): Northern Virginia vacancy ~0.72%, national retail asking ~$196/kW/month, 10MW+ wholesale pricing +19% YoY. Use them to decide which power/margin angle to lead with; write the outreach in segment vocabulary.

Power replaced space as the binding constraint. North America colocation vacancy hit an all-time low (CBRE, year-end 2025); Northern Virginia is ~0.72% in H2 2025. National retail asking rates (250-500kW) are ~$196/kW/month (CBRE H2 2025), the fourth consecutive annual increase, with $200+/kW in primary markets. Ashburn breached $215/kW/month, highest on record. 10MW+ wholesale pricing rose +19% YoY (CBRE H2 2025). 74.3% of under-construction capacity is pre-leased (CBRE 2026 Outlook), and under-construction capacity in primary markets DECLINED year over year for the first time - the build side cracked while demand didn't: roughly half of US data centers slated to open in 2026 are delayed or canceled (transformer lead times to 5 years, grid waits >4 years, FERC interconnection decision due June 2026).

### AI Reshaping the Facility
Legacy 5-7kW racks are obsolete for AI workloads. Current AI standard is ~130kW+/rack today, with the trajectory to 600kW/rack by 2027 (NVIDIA Vera Rubin NVL144 class). NVIDIA GB200 NVL72 runs at ~120kW/rack. Direct-to-chip (D2C) liquid cooling is now the default for new AI racks. Operators without liquid cooling are losing deals  -  Equinix reports 60% of their largest Q4 2025 deals were AI-driven, with 33% higher density than non-AI.
...
Material business risk: ~$130B of projects blocked or delayed per a 2026 study (75 projects blocked in Q1 2026 alone). 188+ organized opposition groups across 40 states. Moratorium bills in 14+ states. Water use is the #1 community concern. Germany mandates 100% renewable energy for DCs from 2027. 74% of providers report customers now demand contractual PUE/carbon commitments. If you can't build out, you must monetize existing footprint harder.
...
The 2026 regulatory fear is no longer only EU extraterritoriality. The pressure is now domestic too. The proposed US GRID Act would set a 20MW federal-review threshold that could force colo business-model restructuring around how large loads interconnect and report. It rides on top of a fragmenting state-and-local landscape - 300+ state-level data-center bills and 100+ local moratoriums in play - so a regulated tenant increasingly has to answer not just where its data sits but which jurisdiction its traffic crossed. Best-effort BGP routing cannot answer that question; policy-controlled, auditable paths can. That makes provable-path control a regulated-workload moat: the colo that can attest the jurisdiction a packet traversed wins the healthcare, financial-services, and government tenants its competitors lose. This is an INTERNAL trigger for picking the regulated-workload angle - keep GRID Act / bill-count specifics out of cold copy and lead with the tenant's own question.

**Cold-safe line (regulated-workload frame, no policy specifics):** "Regulated tenants are starting to ask you to prove which jurisdiction their traffic crossed, not just where the data sits, and best-effort routing cannot answer that. Policy-controlled paths your team provisions and audits under your own brand turn that question into a reason they pick you."
...
### Vertical Integration: Your Fabric Referral Is Now a Compute Competitor (named, June 2026)
The connectivity layer integrated into compute, with dates. Two distinct mechanisms now drive the SAME tenant-loss outcome - and the split matters for live conversations.

**Megaport BUILDS compute.** Megaport acquired bare-metal provider Latitude.sh (closed Nov 2025), launched NVIDIA bare-metal GPU instances in Ashburn and Chicago (Jan 2026), added a storage product (VAST Data deal, June 2026), launched its AIx inference offering, and on June 3, 2026 raised A$827M (~US$594M) to build a globally distributed AI inference cloud across its 1,100+ connected data centers - its homepage now reads "Compute, Network & Storage On Demand." Megaport is now a first-party compute vendor competing for your tenants' GPU and inference spend directly.

**Equinix BROKERS compute.** Equinix is NOT selling its own GPUs. It built a marketplace layer instead: the Distributed AI Hub (Mar 2026) places third-party compute providers next to its tenants, Fabric Intelligence (Apr 2026) routes AI traffic across that ecosystem, and Geo Zones (May 2026) packages jurisdiction-aware placement - all on top of Fabric (bookings +70% YoY, "as our attach rate continues to increase" per its own CFO; sovereignty sold as a premium Fabric tier since May 2026). Equinix doesn't compete for the compute dollar; it owns the marketplace where the tenant goes shopping for it.

**Same outcome, different mechanism - and it's tenant RETENTION, not just margin.** Whether the third party builds the compute (Megaport) or brokers it (Equinix), once you point a tenant there for cloud on-ramp, the tenant ends up shopping for compute and storage inside that third party's marketplace, and the relationship - plus whatever they buy next - goes with them. The referral you send introduces your AI tenants to a platform that owns the next thing they buy. Standing up the interconnection layer under your own brand is what keeps the tenant, and the next purchase, yours. Cold copy keeps this anonymous ("third-party fabric providers," never name Megaport/Equinix); the named mechanism split is for live conversations per `context/core/competitive-positioning.md` and `context/core/differentiation-naas-aggregator.md`.

**Cold-safe line (tenant-retention frame):** "When your tenants need cloud connectivity and you point them to a third party, they end up shopping for compute and storage in someone else's marketplace, and the relationship goes with them. Your team can stand up that interconnection layer under your brand so the tenant, and the next thing they buy, stays yours."

### Inference-Geography Wedge (why-now)
The causal punchline, promoted to a standalone why-now: inference cannot follow training to cheap-power markets. Training is a batch job that tolerates latency, so it chases the cheapest power it can find (rural Texas, Wyoming, Iceland). Inference serves live end users, so it has to sit near them - which means it lands in connectivity-dense metros, not the cheap-power markets out west. The AI tenant comes to your metro because that's where the users are. But the colo in that metro only WINS and KEEPS that tenant if its paths are deterministic - low-jitter, hop-by-hop predictable, private. Compute near the users is table stakes; deterministic private paths between those inference sites and the clouds they call are the differentiator. A facility in the right metro with best-effort routing still loses the tenant to one that can guarantee the path.

**Cold-safe line:** "Inference workloads have to sit close to the users they serve, which is putting AI tenants in your metro, not the cheap-power markets out west. The facility that gives them deterministic private paths is the one that keeps them, and your team can offer that under your own brand."

### Metro-Edge Diffusion
Power constraints in Tier 1 markets (Northern Virginia 0.5% vacancy, moratorium bills in 14+ states) are pushing AI into secondary metros  -  DFW, Columbus, Atlanta, Phoenix, Chicago - and the workload mix reinforces it: inference is now ~two-thirds of AI compute, and the largest lease in Digital Realty's history (200MW, Charlotte, Q1 2026) was an AI INFERENCE deal. Roughly half of 2026-slated US data center openings are delayed or canceled (supersedes the April 2026 "26% facing delays" figure), 75% of enterprise data is captured at the edge (Gartner), and "modular edge compute at hundreds of kilowatts per deployment" (Crusoe Spark) is the shape of the next wave. Regional colos with sub-megawatt to mid-megawatt footprints in these metros are the actual delivery mechanism  -  if they can offer deterministic paths between distributed inference sites. Cologix-class hyperscale facilities explicitly underserve sub-megawatt distributed clusters, leaving that entire layer open to regional operators who can move fast.
...
> **⚠️ Internal angle-selection guide.** Specific figures (Northern Virginia ~0.72% vacancy, national retail ~$196/kW/month, 10MW+ pricing +19% YoY - all CBRE H2 2025; Ashburn $215/kW/month, 113 M&A deals / $69B, 25-30x EV/EBITDA, ~$130B blocked projects, Equinix Fabric bookings +70%) are **internal triggers for picking which angle to lead with**, not customer-facing talking points. Do not cite these numbers in cold outreach or LinkedIn. Use them to determine which relevance bridge fits the account, then write the outreach in segment vocabulary.
...
| Community opposition + supply cracks blocking ~$130B in projects (half of 2026 openings delayed/canceled) | Can't build new facilities fast enough  -  must monetize existing footprint harder | "If you can't build out, build UP. Turn every existing meet-me room into a revenue engine." |
...
| US domestic regulatory pressure (proposed GRID Act 20MW threshold, 300+ state bills, 100+ local moratoriums) | Regulated tenants now ask which jurisdiction their traffic crossed, not just where data sits - and best-effort routing cannot attest it | "Regulated tenants are starting to ask you to prove which jurisdiction their traffic crossed. Policy-controlled paths your team provisions and audits under your own brand turn that question into a reason they pick you." |
| Margin compression (rising power cost, partial pass-through, thinning space-and-power margin) | Standard colo CFO/CRO watches the only line they have get squeezed while interconnection revenue holds its margin | "Power costs keep climbing and most of that lands in space-and-power margin you cannot fully pass through. Lean on interconnection revenue that is not tied to the power bill - your brand, your invoice." |
...
- "The GRID Act and the state bills could force us to restructure how we interconnect large loads  -  what's our exposure?"
- "Inference has to sit near the users, so it's coming to our metro, not the cheap-power markets out west  -  do our paths keep it here?"
- "Power passes through into space-and-power and we still eat the margin  -  interconnection is the only line that holds"
...
*Additive pass 2026-06-14: competitive thesis split (Megaport builds vs Equinix brokers, tenant-retention reframe); inference-geography wedge promoted to standalone why-now + rack-density refresh (~130kW+ today / 600kW by 2027); US GRID-Act domestic-regulatory angle; margin-compression CFO/CRO P&L frame for Standard colo; internal trigger freshness (NoVA ~0.72%, national retail ~$196/kW, 10MW+ +19% YoY, CBRE H2 2025). Pillars, dual-track structure, and "fabric-in-a-box" cold-ban unchanged.*
