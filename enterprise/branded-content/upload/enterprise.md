...
**Lead angle:** Delivery-center reliability across geographies, client data sovereignty (their clients' regulated data riding their network), dark fiber redundancy between primary delivery hubs, audit-ready policy enforcement.
**Persona priority:** VP Network Infrastructure → CSO/CISO → CIO.
...
- **Megaport / Equinix Fabric / PacketFabric:** Coexist. They become a transport option the fabric leverages by API where it makes commercial sense. The enterprise no longer depends on the third-party portal.
...
### Sovereignty is a board-level conversation
EU AI Act fully enforceable August 2026. State privacy laws proliferating. Enterprises are inheriting sovereignty obligations from their customers and regulators. BGP best-effort routing cannot prove where data went. Policy-based path control with jurisdictional audit trails moves from "nice to have" to "compliance gate."
...
- "Our HIPAA Security Rule NPRM compliance plan is being written now - TLS 1.3 and encryption-in-transit are no longer addressable."

**Regulatory drivers + recent incidents to reference:**
HIPAA Security Rule NPRM (Dec 27, 2024) - proposes removing "addressable" flexibility on encryption-in-transit and segmentation, mandates TLS 1.3+; Oracle Cerner deletion incident (April 2025, 39-45 CHS hospitals on paper for 5 days); Ascension Black Basta (May 2024, 5.6M patients, network-segmentation lessons learned); Change Healthcare BlackCat (Feb 2024, 190M records, $3.09B annual hit); Yale New Haven (March 2025); HSCC Sector Mapping toolkit (Oct 2025); HSCC Updated Model Contract Language (Nov 2025); California AB 749 (effective Jan 1, 2025) - zero-trust microsegmentation for connected medical devices at CA hospitals.
