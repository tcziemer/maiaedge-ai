...
- Aqua Comms / DGNet  -  legacy AEC-1/AEC-2/Havfrue/HAVSIL/CC-2 fleet. **Note: 2024 acquisition by EXA Infrastructure  -  verify whether the Aqua Comms record should be reassigned to the EXA parent (which is `Pure Wholesale Carrier - Network Op` / `International Backbone Specialist - Network Op`). If commercially still operating as an independent subsea brand within EXA, classify the child record as N5.**
- Seaborn Networks (Seabras-1, ARBR  -  US-Brazil-Argentina pure-play)
...
- **EXA Infrastructure:** Formerly GTT Infrastructure (separated 2021). Acquired Aqua Comms 2024 (subsea fleet). Pan-EU + transatlantic wholesale fiber. Solid HIGH F4 anchor for EU operations; flag for N4 International Backbone Specialist if the HubSpot record reflects the international/subsea dominant revenue mix.
