...
**Margin-compression CFO/CRO frame (pairs with the "just a landlord" positioning).** Rising power cost passes through into the space-and-power line only partially, thinning that margin every year, while interconnection revenue is priced on connectivity value and is not indexed to the power bill. Lead the finance seat with the line that holds its margin. Cold-safe: "Power costs keep climbing and most of that lands in space-and-power margin you cannot fully pass through, so the operators pulling ahead are leaning on interconnection revenue that is not tied to the power bill. Your team can stand up self-service cross-connects and cloud on-ramps under your brand to grow the line that holds its margin." (Full frame: `colocation.md` § Margin-Compression P&L Frame.)

**Competitive thesis split (Megaport BUILDS vs Equinix BROKERS) — tenant-retention reframe.** Megaport is now a first-party compute vendor (Latitude.sh, bare-metal GPU, AIx, A$827M raise June 2026); Equinix brokers third-party compute via its Distributed AI Hub / Fabric Intelligence / Geo Zones marketplace. Same tenant-loss outcome, different mechanism: point a tenant to either and they end up shopping for compute and storage in someone else's marketplace, relationship included. Cold copy stays anonymous ("third-party fabric providers"); the named split is for live conversations per `context/core/competitive-positioning.md`. Cold-safe: "When your tenants need cloud connectivity and you point them to a third party, they end up shopping for compute and storage in someone else's marketplace, and the relationship goes with them. Your team can stand up that interconnection layer under your brand so the tenant, and the next thing they buy, stays yours."

**US domestic-regulatory angle (regulated-workload moat).** Beyond EU extraterritoriality, the proposed US GRID Act 20MW threshold plus 300+ state bills and 100+ local moratoriums are pushing regulated tenants to ask which jurisdiction their traffic crossed. Policy-controlled, auditable paths are a moat best-effort BGP cannot match. INTERNAL trigger only — keep GRID Act / bill-count specifics out of cold; lead with the tenant's own question. Cold-safe: "Regulated tenants are starting to ask you to prove which jurisdiction their traffic crossed, not just where the data sits, and best-effort routing cannot answer that. Policy-controlled paths your team provisions and audits under your own brand turn that question into a reason they pick you." (Full frame: `colocation.md` § Sovereign Tenant Requirements.)
...
| AI vs general workload | AI/inference workload primary identity. AI rack density is ~130kW+/rack today, trajectory to 600kW/rack by 2027 (NVIDIA Vera Rubin NVL144 class); GB200 NVL72 ~120kW/rack. Direct-to-chip liquid cooling default for new builds. Per `colocation.md`: Equinix reports 60% of largest Q4 2025 deals were AI-driven with 33% higher density. |
...
- **30kW+/rack density** — ~130kW+/rack standard for AI today, 600kW/rack trajectory by 2027.
...
**Inference-geography wedge (why-now).** Inference cannot follow training to cheap-power markets — it serves live users, so it lands in connectivity-dense metros. The AI tenant comes to your metro because that is where the users are, but the colo only KEEPS that tenant if its paths are deterministic. Compute near the users is table stakes; deterministic private paths are the differentiator. Cold-safe: "Inference workloads have to sit close to the users they serve, which is putting AI tenants in your metro, not the cheap-power markets out west. The facility that gives them deterministic private paths is the one that keeps them, and your team can offer that under your own brand." (Full frame: `colocation.md` § Inference-Geography Wedge.)

**Competitive thesis split (Megaport BUILDS vs Equinix BROKERS).** When you refer an AI tenant out for cloud on-ramp, Megaport (first-party compute: Latitude.sh, bare-metal GPU, AIx) or Equinix (brokered compute via Distributed AI Hub / Fabric Intelligence / Geo Zones) puts your tenant in front of the next thing they buy. Same tenant-loss outcome, different mechanism; the frame is retention, not just margin. Cold stays anonymous ("third-party fabric providers") per `context/core/competitive-positioning.md`.
...
- Greenfield enum value: SUPERSEDED RECOMMENDATION (corrected 2026-06-12). `Greenfield` is a REAL cross-segment sub-segment per Cooper's 2026-05-14 decision (pairs with `Data Center Colo Provider` OR `NeoCloud`; actively-being-built, Series A-C funded, sites under construction; R2 auto-migrates to the operational sub-segment when the first site goes live). The earlier archive recommendation in this line is void - segment-classification MAY output `Greenfield` as a final value under those criteria. 2026 reality note: roughly half of 2026-slated US openings are delayed or canceled, so Greenfield S2/S3 windows run LONGER - delayed is the norm, not dead; eviction calls need patience.
