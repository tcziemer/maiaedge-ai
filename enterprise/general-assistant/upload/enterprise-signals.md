...
| Healthcare Systems | HIPAA Security Rule NPRM (Dec 27, 2024) - proposes removing "addressable" flexibility on encryption-in-transit + segmentation, TLS 1.2+ (NOT finalized as of mid-2026; CHIME-led coalition lobbying to withdraw); OCR ransomware consent orders (April 23, 2026, four settlements) whose corrective action plans require network segmentation, asset inventory, and ePHI data-flow mapping - the strongest current hook, enforced rule-or-no-rule; HSCC Sector Mapping & Risk Toolkit (Oct 2025); HSCC Updated Model Contract Language (Nov 2025); HHS OCR breach disclosures (the public "wall of shame"); HITRUST r2 expansions. |
